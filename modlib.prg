...
PROCEDURE MOSTRA_NOME_PROGRAMA(cNomePrograma)
    @ CENTRAL_LIN_INI, CENTRAL_COL_INI + 02 SAY cNomePrograma
RETURN
...
*****************************************************************************
* StrSwap
*   Replaces delimited substrings in cString with elements in aSwap array.
*   Substrings are demited with #{}
* Author:
*   Sergio Lima
* Last update:
*   Aug, 2021
* Example:
*   aNewValues := {"is", "awesome"}
*   ? StrSwap( "This function #{is super} #{cool}!!!", aNewValues )
*   // results
*   // This function is awesome!!!
* params:
*   cString     <expC> - String with data to be changed
*   aSwap       [expA/expC] - String or Array with data to change cString
*   i           <expN> - Function internal use
* Return:
*   expC        cString changed
*****************************************************************************
FUNCTION StrSwap( cString, xSwap, i )
LOCAL aFound := {}, lFewParams := (PCount() < 2)
LOCAL cRegExpFindDelimiter := HB_RegexComp( "\#\{(.*?)\}" )
LOCAL aSwap := iif( ValType(xSwap) == "A", xSwap, {xSwap} )
LOCAL nIndex := iif(ValType(i) == "U", 0, i)

    IF lFewParams .OR. Empty(cString)
        RETURN NIL
    ENDIF
    ...
    cString := StrTran(cString, aFound[1], aSwap[++nIndex])    

    nIndex := iif(nIndex >= LEN(aSwap), 0, nIndex)    
RETURN StrSwap( cString, aSwap, nIndex )
